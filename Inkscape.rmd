...
- Move the page with space+click and drag, or ctrl+right click and drag
- Rotate the page with the rotate tool (bottom right)
...
**Palette:** Basic usage for changing object fill (shift and left-click for stroke/outline). Change palette on the right
...
You can shift+select multiple objects, then right click and group them to move and adjust objects together, and gather them under the same layer controls. 

```{r, results='asis', echo=FALSE}
htmltools::includeHTML("Images/Layers.svg")
```
